...
1. **打席の並べ直し** — スコアブックの成績表は打者ごとに横並びなので、打順の循環を
   使って時系列に戻す。イニング境界は、打席ごとの得点をイニング単位で合計して
   ラインスコアと突き合わせることで確定する。`config/_build_plays.py` が
   打撃成績・イニング別得点・アウト数・投球回・投手別成績のすべてを
   公式記録と照合するので、読み取りを間違えれば落ちる。
...
スコアブックの記号: `BB`四球 `DB`死球 `K`三振 `K'`振り逃げ `1BH`単打 `2BH`二塁打、
`W`ワイルドピッチ `P`パスボールで、続く数字はその打席の失点。
つまり **その打席の得点 = 打点 + W/P の失点**。この試合は東京蓋楡13点のうち6点、
企壮天蓋9点のうち8点が W/P による生還なので、打点だけ見るとハイライトを取り逃がす。
...
python3 tests/test_anchor_suggest.py # 音声からのアンカー推定
python3 config/_build_plays.py --check # 打席データと公式記録の照合
```
