...
- **復習ポリシー（負荷を抑えた多段方式）**: 「全文の解き直し」を毎回短い間隔で出すと負荷が高すぎるため、
  ...
  これにより、新規学習（1周目）中とその直後は「全文復習の洪水」が出ず、近い将来の復習は軽い**ピンのみ復習**が
  担う。旧ポリシー（1→3→7日の短い間隔）で溜まった既存の復習予定は、初回起動時に新ポリシーの長い間隔へ自動移行。
...
- **ピン（苦手な例文）の記録と復習**: PROGRIT のピンは**例文ごと**に手動で立てるもので、レッスン単位では
  ...
  （以前の独立した「📌 ピン復習」一覧セクションは紛らわしいため廃止し、ピン復習は「今日やること」に統合。
  ピンの有無はコース進捗グリッドのドットでも確認できる。）
