...
- **今後の学習計画**: 直近2週間の復習予定を**「全文の解き直し（橙）」と「ピンのみ復習（桃）」の積み上げ棒グラフ**で
  表示（新ポリシーでは近い将来の主役はピン復習のため、両方出さないと予定が空に見える）。現在のペースでの
  初回学習の完了予測つき。
- **ルーティン＋カレンダー**: シャドーイング・ELSA・**英語日記**・業務アウトプット/インプットの日課チェック
  リスト（**2列グリッドでコンパクト表示**。口頭英作文・英語日記は自動判定）。「📅 学習とルーティンの記録」の
  月カレンダーに、**背景の濃さ＝その日の学習量**（旧ヒートマップを統合）と**色ドット＝こなしたルーティン**を
  重ねて可視化（前月へ遡れる）。
- **ダッシュボード構成（ブラッシュアップ済み）**: 記録日 → 今日のプラン（復習・ピン・新規・目安分の統計＋タスク
  リストを一体化）→ ルーティン → 進捗サマリー（学習済x/171・定着・平均定着%・ピン残・連続・直近7日）→
  コース進捗（**コース単位で折りたたみ**。いま手を付けている1コースだけ既定で展開）→ 学習計画 → カレンダー。
