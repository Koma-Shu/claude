...
- **今日のプラン（パート単位でまとめて表示）**: 期限の来た復習と新規を**パート見出しでグループ化**し、
  ...
  各パート見出しの直下に、そのパートの**「ピンのみ復習」を常時表示**（✓で次回へ）。毎回ピン以外も含めて
  復習する負担を避け、ピンのみ復習の回を全文復習と交互に提示する。1日の目安時間・新規上限・学習日数/週・
  コース順は設定で変更可。
- **ピン復習（パート単位）**: PROGRIT のピンは**例文ごと**に手動で立てるもので、レッスン単位では
  復習できないが、**パート単位（Part1 など）で「ピンのみ復習」**ができる。これに合わせ、ピンの本数とメモは
  レッスンごとに記録（モーダルで編集）しつつ、**復習スケジュールはパート単位**（`DB.pinrev[partId]`）で管理。
  毎回ピン以外も含めて復習するのは大変なので、**「ピンのみ復習」する回を「今日やること」に織り込み**、
  通常の復習と交互に提案する。各パートのピン復習は本体より短いスケジュール（1→2→4→8→16→30日）で、
  「📌 ピン復習」セクションに「今日復習 / 次回◯日後」とピンのあるレッスン一覧を表示。「✓ 復習した」で
  次回へ繰り延べ。新しくピンを足すと、そのパートは再び「今すぐ復習待ち」になる。パートのピンが0になると終了。
  旧バージョン（レッスン単位のピン復習）のデータは初回起動時にパート単位へ自動移行し、既存の復習進捗
  （最も復習が遅れているピンに合わせた次回予定）を引き継ぐ（未復習のピンが残るパートのみ「今すぐ」表示）。
